...
class NewsDetailPage extends StatefulWidget {
  String title;
  NewsModel newsModel;
  NewsDetailPage({Key key, @required this.newsModel,@required this.title}) : super(key: key);
...
class _NewsDetailPageState extends State<NewsDetailPage> {
  ...
  void _getBolgContent() async {
    var content =
        await new NewsService().getNewsContent(widget.newsModel.id);
    setState(() {
      widget.newsModel.body = content;
    });
  }

  Future<String> loadHtml() async {
    return await rootBundle.loadString('assets/news.html');
  }
  ...
  void initState() {
    ...
    flutterWebViewPlugin.onStateChanged.listen((state) {
      if (state.type == WebViewState.finishLoad) {
        ...
        var jsonStr = json.encode(widget.newsModel.toJson());
